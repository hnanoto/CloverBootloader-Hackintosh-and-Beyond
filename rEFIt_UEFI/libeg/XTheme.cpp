...
InitTheme(const CHAR8 *ChosenTheme) {
  ...
  if (ThemeNameArray.size() > 0 &&
      (gSettings.GUI.Theme.isEmpty() ||
       StriCmp(gSettings.GUI.Theme.wc_str(), L"embedded") != 0)) {
    ...
    if (ThemeDict == NULL && ChosenTheme) {
      ...
      if (TestTheme.notEmpty()) {
        ...
        if (ThemeDict != NULL) {
          ...
          if (gSettings.GUI.Theme.notEmpty()) {
            DBG("theme %s chosen from nvram is absent, using theme defined in "
                "config: %ls\n",
      ...
      // FreePool(ChosenTheme); // ChosenTheme is an argument passed here, so
      // the callee is "own" that memory. ChosenTheme = NULL;    // Why is this
      // bad pratice : it's assuming that ChosenTheme was dynamically allocated
      // and freeable. What is this the content of an XString ?
    }
...
finish:
  if (!ThemeDict) { // No theme could be loaded, use embedded
    DBG(" using embedded theme (Forced Dark Mode)\n");
    if (ThemeX->DarkEmbedded) { // when using embedded, set Daylight according
                                // to darkembedded
      ThemeX->Daylight = false;
    } else {
      ThemeX->Daylight = false; // Default to Dark Mode for embedded theme
    }
    ...
    // fill some fields
    // ThemeX->Font = FONT_ALFA; //to be inverted later. At start we have
    // FONT_GRAY
  ...
  if (ChosenTheme != NULL) {
    // FreePool(ChosenTheme); // ChosenTheme is an argument passed here, so the
    // callee is "own" that memory.
    //  Why is this bad pratice : it's assuming that ChosenTheme was dynamically
    //  allocated and freeable. What is this the content of an XString ?
  }
  if (!ThemeX->TypeSVG) {
    ThemeX->PrepareFont();
  }
...
// xtheme class
XTheme::XTheme()
    : Icons(), ThemeDir(0), HideBadges(0), HideUIFlags(0), Font(FONT_ALFA),
      CharWidth(0), SelectionColor(0), FontFileName(), Theme(),
      BannerFileName(), SelectionSmallFileName(), SelectionBigFileName(),
      SelectionIndicatorName(), DefaultSelection(), BackgroundName(),
      BackgroundScale(imNone), BackgroundSharp(0), BackgroundDark(false),
      SelectionOnTop(false), BootCampStyle(false), BadgeOffsetX(0),
      BadgeOffsetY(0), BadgeScale(0), ThemeDesignWidth(0), ThemeDesignHeight(0),
      BannerPosX(0), BannerPosY(0), BannerEdgeHorizontal(0),
      BannerEdgeVertical(0), BannerNudgeX(0), BannerNudgeY(0),
      VerticalLayout(false), NonSelectedGrey(false), MainEntriesSize(0),
      TileXSpace(0), TileYSpace(0), Proportional(false), embedded(false),
      DarkEmbedded(false), TypeSVG(false), Scale(0), CentreShift(0),
      row0TileSize(0), row1TileSize(0), BanHeight(0), LayoutHeight(0),
      LayoutBannerOffset(0), LayoutButtonOffset(0), LayoutTextOffset(0),
      LayoutAnimMoveForMenuX(0), ScrollWidth(0), ScrollButtonsHeight(0),
      ScrollBarDecorationsHeight(0), ScrollScrollDecorationsHeight(0),
      FontWidth(0), FontHeight(0), TextHeight(0), Daylight(false), Background(),
      BigBack(), Banner(), SelectionImages(), Buttons(),
      ScrollbarBackgroundImage(), BarStartImage(), BarEndImage(),
      ScrollbarImage(), ScrollStartImage(), ScrollEndImage(), UpButtonImage(),
      DownButtonImage(), FontImage(), BannerPlace(), Cinema() {
  Init();
}
...
void XTheme::Init() {
  ...
  //  CustomIcons = false;          //TODO don't know how to handle with SVG
  //  theme
  ...
  Proportional = false;
  //  ShowOptimus = false;
  //  DarkEmbedded = false;  //looks like redundant, we always check Night or
  //  Daylight
  TypeSVG = false;
  //  Codepage = 0xC0;           //this is for PNG theme
  //  CodepageSize = 0xC0;           // INTN        CodepageSize; //extended
  //  latin
  Scale = 1.0f;
  CentreShift = 0.0f;
  Daylight = true;
...
TagDict *XTheme::LoadTheme(const XStringW &TestTheme) {
  ...
  Status =
      self.getThemesDir().Open(&self.getThemesDir(), &ThemeDir,
                               m_ThemePath.wc_str(), EFI_FILE_MODE_READ, 0);
  if (EFI_ERROR(Status)) {
    if (ThemeDir != NULL) {
...
XTheme::GetThemeTagSettings(const TagDict *DictPointer) {
  ...
  // if NULL parameter, quit after setting default values, this is embedded
  // theme
  if (DictPointer == NULL) {
    return EFI_SUCCESS;
  }
  ...
  if (Dict != NULL) {
    ...
    if (Prop != NULL && (Prop->isString()) &&
      ...
      if ((Prop->getString()->stringValue()[0] == 'S') ||
          (Prop->getString()->stringValue()[0] == 's')) {
        BackgroundScale = imScale;
      } else if ((Prop->getString()->stringValue()[0] == 'T') ||
                 (Prop->getString()->stringValue()[0] == 't')) {
  ...
  if (Prop != NULL) {
    ...
    if (Prop->isString() && Prop->getString()->stringValue().notEmpty()) {
      ...
      if (Prop2 != NULL && (Prop2->isString()) &&
          Prop2->getString()->stringValue().notEmpty()) {
  ...
  if (Dict != NULL) {
    ...
    ScrollButtonsHeight =
        (UINTN)GetPropertyAsInteger(Prop, ScrollButtonsHeight);

    Prop = Dict->propertyForKey("BarHeight");
    ScrollBarDecorationsHeight =
        (UINTN)GetPropertyAsInteger(Prop, ScrollBarDecorationsHeight);

    Prop = Dict->propertyForKey("ScrollHeight");
    ScrollScrollDecorationsHeight =
        (UINTN)GetPropertyAsInteger(Prop, ScrollScrollDecorationsHeight);
  }
  ...
  if (Dict != NULL) {
    ...
    if (Prop != NULL && (Prop->isString()) &&
        Prop->getString()->stringValue().notEmpty()) {
      if ((Prop->getString()->stringValue()[0] == 'A') ||
          (Prop->getString()->stringValue()[0] == 'B')) {
        Font = FONT_ALFA;
      } else if ((Prop->getString()->stringValue()[0] == 'G') ||
                 (Prop->getString()->stringValue()[0] == 'W')) {
        Font = FONT_GRAY;
      } else if ((Prop->getString()->stringValue()[0] == 'L') ||
                 (Prop->getString()->stringValue()[0] == 'l')) {
        Font = FONT_LOAD;
      }
    }
    if (Font == FONT_LOAD) {
  ...
  const TagArray *AnimeArray =
      DictPointer->arrayPropertyForKey("Anime"); // array of dict
  if (AnimeArray != NULL) {
    ...
    for (INTN i = 0; i < Count; i++) {
      ...
      NewFilm->SetIndex(
          (UINTN)GetPropertyAsInteger(Prop, 1)); // default=main screen
      ...
      NewFilm->FrameTime =
          (UINTN)GetPropertyAsInteger(Prop, 50); // default will be 50ms
...
const XIcon &XTheme::GetIcon(INTN Id) { return GetIconAlt(Id, -1); }

/*
 * Get Icon with this ID=id, for example VOL_INTERNAL_HFS
 * if not found then search for ID=Alt with Native attribute set, for example
 * VOL_INTERNAL if not found then check embedded with ID=Id if not found then
 * check embedded with ID=Alt
 */
XIcon &XTheme::GetIconAlt(INTN Id, INTN Alt) // if not found then take embedded
{
  ...
  for (size_t i = 0;
       i < Icons.size() && (IdFound < 0 || (Alt >= 0 && AltFound < 0)); i++) {
    if (Icons[i].Id == Id) {
      IdFound = i;
    }
    if (Icons[i].Id == Alt) {
  ...
  if (IdFound >= 0 && Icons[IdFound].Image.isEmpty()) {
    ...
    if (Alt >= 0 && AltFound >= 0 && Icons[AltFound].Native &&
        !Icons[AltFound].Image.isEmpty()) {
      // using Alt icon
      ...
      delete NewIcon; // there is probably a way better way to do this. By
                      // initializing directly Icons[IdFound].Image instead of
                      // using dynamically allocated copy.
    }
  }
...
const XIcon &XTheme::LoadOSIcon(const XString8 &Full) {
  // input value can be L"win", L"ubuntu,linux", L"moja,mac" set by
  // GetOSIconName (OSVersion)
  XString8 First;
  XString8 Second;
  XString8 Third;
  ...
  if (Comma != MAX_XSIZE) { // Comma
    ...
    if (Comma == MAX_XSIZE) {
      ...
      if (!ReturnIcon->isEmpty())
        return *ReturnIcon;
      Third =
          "os_"_XS8 + Second.subString(SecondComma + 1, Size - SecondComma - 1);
...
void XTheme::FillByEmbedded() {
  ...
  // and buttons
  Buttons[0].FromPNG(ACCESS_EMB_DATA(emb_radio_button),
                     ACCESS_EMB_SIZE(emb_radio_button));
  Buttons[1].FromPNG(ACCESS_EMB_DATA(emb_radio_button_selected),
                     ACCESS_EMB_SIZE(emb_radio_button_selected));
  Buttons[2].FromPNG(ACCESS_EMB_DATA(emb_checkbox),
                     ACCESS_EMB_SIZE(emb_checkbox));
  Buttons[3].FromPNG(ACCESS_EMB_DATA(emb_checkbox_checked),
                     ACCESS_EMB_SIZE(emb_checkbox_checked));

  if (Daylight) {
    SelectionImages[0].FromPNG(ACCESS_EMB_DATA(emb_selection_big),
                               ACCESS_EMB_SIZE(emb_selection_big));
    SelectionImages[2].FromPNG(ACCESS_EMB_DATA(emb_selection_small),
                               ACCESS_EMB_SIZE(emb_selection_small));
  } else {
    SelectionImages[0].FromPNG(ACCESS_EMB_DATA(emb_dark_selection_big),
                               ACCESS_EMB_SIZE(emb_dark_selection_big));
    SelectionImages[2].FromPNG(ACCESS_EMB_DATA(emb_dark_selection_small),
                               ACCESS_EMB_SIZE(emb_dark_selection_small));
  }

  SelectionImages[4].FromPNG(ACCESS_EMB_DATA(emb_selection_indicator),
                             ACCESS_EMB_SIZE(emb_selection_indicator));
}

void XTheme::ClearScreen() // and restore background and banner
{
  if (UGAWidth == 0 || UGAHeight == 0) {
    // jief : I had the case where no graphic protocol were availbale. So
    // UGAWidth == 0 && UGAHeight == 0 which panic because Background would be
    // empty.
    //        Background.GetPixelPtr(0, 0) panic on an empty image because there
    //        is no pixel at x=0 y=0
    // test could be "if ( UGAWidth == 0 && UGAHeight == 0 )" (&& instead of
    // ||), but I feel like if one of the dimension is 0, we have a graphic
    // problem. also, should we implement the case if we are in text mode ?
    return;
  }
  ...
  if (!(HideUIFlags & HIDEUI_FLAG_BANNER)) {
    ...
    if (!Banner.isEmpty()) {
      ...
      BannerPlace.Height =
          (BanHeight >= Banner.GetHeight()) ? Banner.GetHeight() : BanHeight;
      ...
      if (!TypeSVG) {
        ...
        if ((BannerPosX >= 0 && BannerPosX <= 1000) &&
            (BannerPosY >= 0 && BannerPosY <= 1000)) {
          // Check if screen size being used is different from theme origination
          // size. If yes, then recalculate the placement % value. This is
          // necessary because screen can be a different size, but banner is not
          // scaled.
          BannerPlace.XPos = HybridRepositioning(BannerEdgeHorizontal,
                                                 BannerPosX, BannerPlace.Width,
                                                 UGAWidth, ThemeDesignWidth);
          BannerPlace.YPos = HybridRepositioning(BannerEdgeVertical, BannerPosY,
                                                 BannerPlace.Height, UGAHeight,
                                                 ThemeDesignHeight);
          // Check if banner is required to be nudged.
          BannerPlace.XPos = CalculateNudgePosition(
              BannerPlace.XPos, BannerNudgeX, Banner.GetWidth(), UGAWidth);
          BannerPlace.YPos = CalculateNudgePosition(
              BannerPlace.YPos, BannerNudgeY, Banner.GetHeight(), UGAHeight);
          //         DBG("banner position new style\n");
        } else {
          // Use rEFIt default (no placement values speicifed)
          BannerPlace.XPos = (UGAWidth >= Banner.GetWidth())
                                 ? (UGAWidth - Banner.GetWidth()) >> 1
                                 : 0;
          BannerPlace.YPos = (BanHeight >= Banner.GetHeight())
                                 ? (BanHeight - Banner.GetHeight())
                                 : 0;
  ...
  if (Background.GetWidth() != UGAWidth ||
      Background.GetHeight() !=
          UGAHeight) { // should we type UGAWidth and UGAHeight as UINTN to
                       // avoid cast ?
  ...
  if (!BigBack.isEmpty()) {
    ...
    case imTile: {
      INTN x = (BigBack.GetWidth() * ((UGAWidth - 1) / BigBack.GetWidth() + 1) -
                UGAWidth) >>
               1;
      INTN y =
          (BigBack.GetHeight() * ((UGAHeight - 1) / BigBack.GetHeight() + 1) -
           UGAHeight) >>
          1;
      EFI_GRAPHICS_OUTPUT_BLT_PIXEL *p1 = Background.GetPixelPtr(0, 0);
      for (INTN j = 0; j < UGAHeight; j++) {
        for (INTN i = 0; i < UGAWidth; i++) {
          *p1++ = BigBack.GetPixel((i + x) % BigBack.GetWidth(),
                                   (j + y) % BigBack.GetHeight());
        }
      }
      //      DBG("back copy tiled\n");
      break;
    ...
    default:
      // already scaled
      Background = BigBack;
      // DBG("Assign Background = BigBack.  BigBack.Width=%lld
    ...
    Background.Fill(
        BlueBackgroundPixel); // blue opaque. May be better to set black opaque?
  }
...
void XTheme::FillByDir() // assume ThemeDir is defined by InitTheme() procedure
{
  ...
  for (INTN i = 0; i < IconsNamesSize; ++i) { // scan full table
    ...
    if (!EFI_ERROR(Status)) {
      NewIcon->ImageNight.LoadXImage(
          ThemeDir, SWPrintf("%s_night", IconsNames[i].c_str()));
    }
    Icons.AddReference(NewIcon, true);
    if (EFI_ERROR(Status)) {
      if (i >= BUILTIN_ICON_VOL_INTERNAL_HFS &&
...
void XTheme::InitBar() {
  if (!TypeSVG) {
    ...
    ScrollbarBackgroundImage =
        GetIcon("scrollbar_background"_XS8).GetBest(!Daylight);
    BarStartImage.setEmpty();
    BarEndImage.setEmpty();
    ScrollbarImage = GetIcon("scrollbar_holder"_XS8)
                         .GetBest(!Daylight); //"_night" is already accounting
  ...
  if (!TypeSVG) {
    ...
    // Question: why we don't want these for svg? (upbutton, downbutton,
    // scrollstart, scrollend - also have hardcoded 0 height in
    // REFIT_MENU_SCREEN.cpp)
    if (BarStartImage.isEmpty()) {
      BarStartImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_bar_start),
                            ACCESS_EMB_SIZE(emb_scroll_bar_start));
    }
    if (BarEndImage.isEmpty()) {
      BarEndImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_bar_end),
                          ACCESS_EMB_SIZE(emb_scroll_bar_end));
    }
    if (ScrollStartImage.isEmpty()) {
      ScrollStartImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_scroll_start),
                               ACCESS_EMB_SIZE(emb_scroll_scroll_start));
    }
    if (ScrollEndImage.isEmpty()) {
      ScrollEndImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_scroll_end),
                             ACCESS_EMB_SIZE(emb_scroll_scroll_end));
    }
    if (UpButtonImage.isEmpty()) {
      UpButtonImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_up_button),
                            ACCESS_EMB_SIZE(emb_scroll_up_button));
    }
    if (DownButtonImage.isEmpty()) {
      DownButtonImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_down_button),
                              ACCESS_EMB_SIZE(emb_scroll_down_button));
    }
  }

  // fill these from embedded for both svg and non-svg
  if (ScrollbarBackgroundImage.isEmpty()) {
    ScrollbarBackgroundImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_bar_fill),
                                     ACCESS_EMB_SIZE(emb_scroll_bar_fill));
  }
  if (ScrollbarImage.isEmpty()) {
    ScrollbarImage.FromPNG(ACCESS_EMB_DATA(emb_scroll_scroll_fill),
                           ACCESS_EMB_SIZE(emb_scroll_scroll_fill));
  }
}
...
void XTheme::FillRectAreaOfScreen(IN INTN XPos, IN INTN YPos, IN INTN Width,
                                  IN INTN Height) {
